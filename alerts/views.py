...
class Sync(views.APIView):
    ...
    def get(self, request):
        ...
        if not alerts_qs.exists():
            return response.Response({"message": "No Alerts Scheduled."}, status.HTTP_200_OK)

        URL = "https://x.wazirx.com/wazirx-falcon/api/v2.0/crypto_rates"
        res = requests.get(URL)

        if res.status_code != 200:
            return response.Response({"message": "NOT OK"}, status.HTTP_503_SERVICE_UNAVAILABLE)
        
        data = res.json()        

        currencies = currencies_models.Currency.objects.filter(is_active=True).values_list("code", flat=True)
        ...
        if not stack:
            return response.Response({"message": "No Alerts to send."}, status.HTTP_200_OK)
        ...
        try:
            ...
            pass
        return response.Response({"message": "Alert Sent"}, status.HTTP_200_OK)
